...
const getUrlFromClusterRef = async (
  // TODO: Try to put template action params
...
export function createKubernetesNamespaceAction(catalogClient: CatalogClient) {
  return createTemplateAction<TemplateActionParameters>({
    ...
    async handler(ctx) {
      const { namespaceName, clusterRef, token, url, skipTLSVerify, caData } =
        ctx.input;
      ...
      await api.createNamespace(namespace).catch((e: HttpError) => {
